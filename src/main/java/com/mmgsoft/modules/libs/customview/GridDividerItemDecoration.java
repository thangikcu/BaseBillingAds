...
/**
 * Created by khoahihi on 9/22/2017.
 * SavvyCom
...
public class GridDividerItemDecoration extends RecyclerView.ItemDecoration {
	...
	private void drawHorizontalDividers(Canvas canvas, RecyclerView parent) {
		...
		for (int i = 1; i < mNumColumns; i++) {
			...
			int dividerTop = firstRowChild == null ? 0 : firstRowChild.getTop();
			int dividerRight = firstRowChild == null ? 0 : firstRowChild.getLeft();
			int dividerLeft = dividerRight - mHorizontalDivider.getIntrinsicWidth();
			int dividerBottom = lastRowChild == null ? 0 : lastRowChild.getBottom();
	...
	private void drawVerticalDividers(Canvas canvas, RecyclerView parent) {
		...
		for (int i = 1; i <= rowCount; i++) {
			...
			int dividerLeft = leftmostChild == null ? 0 : leftmostChild.getLeft();
			int dividerBottom = leftmostChild == null ? 0 : leftmostChild.getTop();
			int dividerTop = dividerBottom - mVerticalDivider.getIntrinsicHeight();
			int dividerRight = rightmostChild == null ? 0 : rightmostChild.getRight();
