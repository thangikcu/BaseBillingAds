...
ECHO project.afterEvaluate { preBuild.dependsOn { taskBeforeBuild(); generatePostData; if (providers.gradleProperty("GENERATE_KEYSTORE").getOrElse("false").toBoolean()) generateKeystore(GOOGLE_APPLICATION_ID, AMAZON_APPLICATION_ID) }; assembleAmazonDebug.doLast { taskAfterBuild(); getPermissions(ARCHIVES_BASE_NAME) }; }>> build.gradle
