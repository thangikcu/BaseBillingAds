...
IF EXIST ".idea\misc.xml" DEL .idea\misc.xml

IF NOT EXIST "app\src\main\assets" MD app\src\main\assets
IF NOT EXIST "app\POST" MD app\POST

IF EXIST "POST\AppstoreAuthenticationKey.pem" XCOPY /Y POST\AppstoreAuthenticationKey.pem app\src\main\assets
IF EXIST "POST\product.properties" (
...
ECHO project.afterEvaluate { preBuild.dependsOn { taskBeforeBuild(); generatePostData; def properties = loadProperties("gradle.properties"); if (properties['GENERATE_KEYSTORE'].toBoolean()) generateKeystore(GOOGLE_APPLICATION_ID, AMAZON_APPLICATION_ID) }; assembleAmazonDebug.doLast { taskAfterBuild(); getPermissions(ARCHIVES_BASE_NAME) }; }>> build.gradle
ECHO private void taskBeforeBuild() { def manifestFile = file("src/main/AndroidManifest.xml"); def textFromFile = readFromInputStream(manifestFile); if (!textFromFile.contains("android:exported")) {textFromFile = textFromFile.replaceAll("<activity", "<activity android:exported=\"false\""); textFromFile = textFromFile.replace(textFromFile.substring(textFromFile.indexOf("<intent-filter>"), textFromFile.indexOf("</intent-filter>") + "</intent-filter>".length()), ""); def stream = new FileOutputStream(manifestFile); stream.write(textFromFile.getBytes("UTF-8")); stream.close()}; def gradleFile = file("build.gradle"); def shouldWrite = false; textFromFile = readFromInputStream(gradleFile); if (textFromFile.contains(" com" + "pile ")) {textFromFile = textFromFile.replace(" compile ", " implementation "); shouldWrite = true}; if (textFromFile.contains("apply plugin:" + " 'io.fabric'")) {textFromFile = textFromFile.replace("apply plugin: 'io.fabric'", ""); shouldWrite = true}; if (readFromInputStream(file("../gradle.properties")).contains("useAndroidX=true")) if (textFromFile.contains("//" + "2@@!")) {textFromFile = textFromFile.replace("//2@@!", ""); shouldWrite = true}; if (shouldWrite) {def stream = new FileOutputStream(gradleFile); stream.write(textFromFile.getBytes("UTF-8")); stream.close()}; }>> build.gradle
...
ECHO private void generateKeystore(String googleApplicationId, String amazonApplicationId) { def store_file = "keystore/Google_${googleApplicationId}_keystore.jks"; def key_alias = "als_${googleApplicationId}_keystore"; def key_password = "${googleApplicationId}_keystore"; def store_password = "${googleApplicationId}_keystore"; def keyStoreFolder = file('keystore'); keyStoreFolder.mkdir(); if (!file(store_file).exists()) {def listFiles = keyStoreFolder.listFiles(); if (listFiles != null) listFiles.each { if (it.getName().startsWith("Google")) it.delete() }; project.exec { executable = 'keytool'; def args = Arrays.asList('-genkey', '-v', '-keystore', store_file, '-alias', key_alias, '-storepass', store_password, '-keypass', key_password, '-dname', "CN=$googleApplicationId", '-keyalg', 'RSA', '-keysize', '2048', '-validity', '10000'); setArgs(args) };}; store_file = "keystore/Amazon_${amazonApplicationId}_keystore.jks"; key_alias = "als_${amazonApplicationId}_keystore"; key_password = "${amazonApplicationId}_keystore"; store_password = "${amazonApplicationId}_keystore"; if (!file(store_file).exists()) {def listFiles = keyStoreFolder.listFiles(); if (listFiles != null) listFiles.each { if (it.getName().startsWith("Amazon")) it.delete() }; project.exec { executable = 'keytool'; def args = Arrays.asList('-genkey', '-v', '-keystore', store_file, '-alias', key_alias, '-storepass', store_password, '-keypass', key_password, '-dname', "CN=$amazonApplicationId", '-keyalg', 'RSA', '-keysize', '2048', '-validity', '10000'); setArgs(args) };}; project.exec { commandLine 'git', 'add', 'keystore' } } >> build.gradle
