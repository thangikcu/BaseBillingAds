...
ECHO task getPermissions { def apkPath = "build/outputs/apk/debug/${ARCHIVES_BASE_NAME}-debug.apk"; if (file(apkPath).exists()) { def file = file("POST/Permissions.txt"); file.delete(); file.createNewFile(); def byteArrayOutputStream = new ByteArrayOutputStream(); project.exec { setStandardOutput(byteArrayOutputStream); commandLine 'aapt', 'd', 'permissions', apkPath }; def stream = new FileOutputStream(file); stream.write(byteArrayOutputStream.toString().getBytes("UTF-8")); stream.close() } }>> build.gradle
